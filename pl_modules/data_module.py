...
import fastmri
from data.mri_data import CombinedCmrxReconSliceDataset, CmrxReconSliceDataset, CombinedFastmriKneeSliceDataset, FastmriKneeSliceDataset
...
def worker_init_fn_fastmri_knee(worker_id):
    """Handle random seeding for all mask_func."""
    worker_info = torch.utils.data.get_worker_info()
    data: Union[
        FastmriKneeSliceDataset, CombinedFastmriKneeSliceDataset
    ] = worker_info.dataset  # pylint: disable=no-member
    ...
    if isinstance(data, CombinedFastmriKneeSliceDataset):
        for i, dataset in enumerate(data.datasets):
            if dataset.transform.mask_func is not None:
                if (
                    is_ddp
...
class FastmriKneeDataModule(pl.LightningDataModule):
    """
    Data module class for fastMRI data sets.
    ...
    def _create_data_loader(
        ...
        # if desired, combine train and val together for the train split
        dataset: Union[FastmriKneeSliceDataset, CombinedFastmriKneeSliceDataset]
        if is_train and self.combine_train_val:
            data_paths = [
            ...
            if volume_sample_rate is not None:
                volume_sample_rates = [volume_sample_rate, volume_sample_rate]
            dataset = CombinedFastmriKneeSliceDataset(
                roots=data_paths,
                transforms=data_transforms,
                challenges=challenges,
                sample_rates=sample_rates,
        ...
        dataloader = torch.utils.data.DataLoader(
            ...
            batch_size=self.batch_size,
            num_workers=self.num_workers,
            worker_init_fn=worker_init_fn_fastmri_knee,
            sampler=sampler,
            shuffle=is_train if sampler is None else False,
        )
